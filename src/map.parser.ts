...
function convertToObject(data) {
    ...
    Object.entries(data).forEach(([key, value]) => {
        newObj[key] = convertToObject(value);
    });
...
function encodeNode(node, fullDetail = false) {
    if (!fullDetail) {
        return {
            id: node.id,
            x: node.coordinates ? node.coordinates.x : 0,
            y: node.coordinates ? node.coordinates.y : 0,
            icon: node.icon || "🏥" // Default icon if not specified
        };
    }
    const metadataEncoded = node.metadata ? node.metadata.map(metadata => convertToObject(metadata)) : [];
    return {
        id: node.id,
        name: node.name,
...
function run() {
    ...
    console.log("Demo of Object encoder",encodeNode(pathway.decision[0], true));
