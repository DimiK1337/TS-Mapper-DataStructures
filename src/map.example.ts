...
// Define the distribution type for arrival and duration times
type Distribution = {
    distribution: string;
    parameters: Array<{ [key: string]: number }>;
};

// Define the metadata type for workflow steps, decisions, and delays
type Metadata = {
...
// Define the node type for start, workflow steps, decision points, delays, and end points
type NodeType = {
    id: string;
    name: string;
    incoming?: string;
...
// Define the types for the path connections between nodes
type Path = {
    id: string;
    name: string;
    source: string;
...
// Define the overall pathway structure containing all components
